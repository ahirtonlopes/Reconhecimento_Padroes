...
def modelo_para_frente_L(X, parameters):
    ...
    for l in range(1, L):
        A_prev = A 
        A, cache = para_frente_linear_ativacao(A_prev, parameters['W' + str(l)], parameters['b' + str(l)], activation = "relu")
        caches.append(cache)
...
def modelo_para_tras_L(AL, Y, caches):
    ...
    grads["dA" + str(L)], grads["dW" + str(L)], grads["db" + str(L)] = para_tras_linear_ativacao(dAL, current_cache, activation = "sigmoid")
    
    for l in reversed(range(L-1)):
        # lth layer: (RELU -> LINEAR) gradients.
        current_cache = caches[l]
        dA_prev_temp, dW_temp, db_temp = para_tras_linear_ativacao(grads["dA" + str(l + 2)], current_cache, activation = "relu")
...
def atualiza_parametros(parameters, grads, learning_rate):
    """
